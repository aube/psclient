...
- **`const/base.tailwind.js`** — Tailwind theme config. CSS is **generated server-side** by a TWCSS server (not CLI). New utility classes must be added to `TW_CLASSES_SAFELIST`.

## Key conventions

- **Forms**: Add `class="form"`. Submission is auto-bound in `/static/main.js` via `POST /api/v1/interactions`.
- **CSS theming**: Use `.inverted` for dark sections. Use `.btn-primary`, `.btn-secondary`, `.card`, `.form-input`, `.form-label`, `.checkbox-*`, `.btn-submit` component classes defined in `base.tailwind.js`.
- **Styling**: New Tailwind utility classes used in dynamically-created elements (JS/DOM) or in form HTML templates must be added to `TW_CLASSES_SAFELIST` in `const/base.tailwind.js`. Classes used in server-side templates that go through `@apply` in `TW_BASE_CSS` are safe.
...
- **CSS regen on every F5**: Fixed. `fetchTemplatesLast` now returns `false` for `[]`. Hash comparison in `cssTWRegenerate` / `cssTemplatesRegenerate` prevents actual regen.
