...
import { translitRu2En } from '../../lib/utils'
...
const uploadStatus = ref<ImageStatus | null>(null)
...
const uploadFiles = async () => {
  ...
  try {
    ...
    const uploadPromises = selectedFiles.value.map(async (file) => {
      ...
      formData.append('url', translitRu2En(file.name).toLocaleLowerCase().replaceAll(/\s/g, "-"))
  ...
  <div class="upload-container">
    ...
    >
      <div class="drop-zone-content">
        ...
        <p class="drop-hint">
          Поддерживаемые форматы: JPG, JPEG, PNG, GIF, SVG, WEBP
        </p>
      </div>
      <input
        ref="fileInput"
