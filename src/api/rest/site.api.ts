...
import { Site, SiteNew, Sites, Pagination } from '../../types'
...
export const useSiteAPI = () => {
  ...
  const list = async (page: number = 0, size: number = 100): Promise<{rows:Sites; pagination:Pagination}> => {
    ...
    const response = await get<{rows:Sites; pagination:Pagination}>('/api/v1/sites' + (queryString ? `?${queryString}` : ''))
    if (!response.data) {
      if (response.error)
        throw Error(response.error)
